#!/usr/bin/env python3
"""Build a styled HTML and a self-contained PDF for the 'Nourish' ebook.
...
SRC = "nourish-ebook.md"
HTML_OUT = "Nourish-Pregnancy-Nutrition-Companion.html"
PDF_OUT = "Nourish-Pregnancy-Nutrition-Companion.pdf"
...
<title>Nourish — Pregnancy Nutrition Companion</title>
<style>
 ...
 h1.booktitle{{font-size:3em;text-align:center;color:#2e7d32;margin:.1em 0;}}
 p.subtitle{{text-align:center;font-size:1.4em;color:#c0392b;font-weight:bold;margin:.2em 0;}}
 p.tagline{{text-align:center;font-size:1.05em;color:#555;font-style:italic;margin:.2em 0 1em;}}
 h1.part{{font-size:2em;color:#fff;background:#2e7d32;padding:14px 18px;border-radius:6px;
   margin-top:2em;}}
 h2{{font-size:1.5em;color:#2e7d32;margin-top:1.8em;border-bottom:2px solid #d7ead8;padding-bottom:.2em;}}
 h3{{font-size:1.15em;color:#c0392b;margin-top:1.4em;}}
 ...
 .callout{{background:#eef6ef;border-left:5px solid #2e7d32;padding:12px 18px;margin:1.4em 0;border-radius:5px;}}
 ...
 th,td{{border:1px solid #cbd8cc;padding:7px 9px;text-align:left;vertical-align:top;}}
 th{{background:#2e7d32;color:#fff;}} tr:nth-child(even) td{{background:#f4faf4;}}
    ...
    if kind == "hr":
        ...
        if not title_seen:
            ...
            for wl in wrap(clean(payload), 40, "B", CW):
                ln(wl, "B", 40, 48, center=True, color=(0.18, 0.49, 0.20))
            title_seen = True
            sp(6)
        else:
            sp(20)
        ...
        if bi <= 2:
            for wl in wrap(clean(payload), 19, "B", CW):
                ln(wl, "B", 19, 25, center=True, color=(0.75, 0.22, 0.17))
            sp(4)
        else:
            sp(16)
            for wl in wrap(clean(payload), 17, "B", CW):
                ln(wl, "B", 17, 23, color=(0.18, 0.49, 0.20))
            sp(4)
    elif kind == "h3":
        if bi <= 3:
            ...
            for wl in wrap(clean(payload), 13, "B", CW):
                ln(wl, "B", 13, 19, color=(0.75, 0.22, 0.17))
            sp(3)
    elif kind == "p":
        ...
        emit_row(header + [""] * (ncol - len(header)), True, (0.18, 0.49, 0.20))
        for ri, r in enumerate(rows):
            r = (r + [""] * ncol)[:ncol]
            shade = (0.95, 0.98, 0.95) if ri % 2 else None
            emit_row(r, False, shade)
        sp(8)
    elif kind == "quote":
        sub = payload
        story = quote_is_story(sub)
        col = (1.0, 0.965, 0.917) if story else (0.933, 0.965, 0.937)
        sp(3)
        box_start(col)
        first = True
        for skind, stext in sub:
